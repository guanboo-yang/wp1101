...
const Game = () => {
    ...
    useEffect(() => {
        ...
        if (sprites.sprites && sprites.sprites.length > 0)
            changeCamera(getSpriteByType(sprites.sprites, 'ship'))
            ...
            sprites.sprites
                ...
                .forEach((sprite) => {
                    ...
                    if (state.objects[sprite.id]) {
                        ...
                        switch (sprite.type) {
                            case 'ship':
                                state.objects[sprite.id] = new Ship(
                                    sprite.id,
                                    sprite.color,
                                    sprite.pos,
                                    sprite.angle
                                ...
                                break
                            case 'bullet':
                                state.objects[sprite.id] = new Bullet(
                                    sprite.pos,
                                    sprite.angle
                                )
                                break
                            case 'wall':
                                state.objects[sprite.id] = new Wall(
                                    sprite.pos,
                                    sprite.breakable
                                )
                                break
    ...
    useEffect(() => {
        if (text && sprites.bounds) {
            ...
            setTexts((texts) => [
                ...texts,
                new Text(
                    { x: sprites.bounds[0] / 2, y: sprites.bounds[1] / 2 },
                    'red',
                    text
                )
            ])
    ...
    return (
        <div align="center">
            <canvas
                ...
                style={{
                    background:
                        'radial-gradient(circle, #080430 0%, #000000 100%)',
                    borderRadius: '10px'
                }}
