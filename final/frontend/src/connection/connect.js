...
const client = new WebSocket(
    userProfile
        ? `${process.env.REACT_APP_WEBSOCKET_URL}?name=${userProfile.name}`
...
const useConnection = () => {
    ...
    client.onmessage = async (byteString) => {
        ...
        switch (task) {
            ...
            case 'createFail':
                showMessage(
                    'The Username or the Email has been taken',
                    'error',
                    2000
                )
                break
            ...
            case 'updatedPosition':
                ...
                if (payLoad.newHost) {
                    setRoom({
                        ...
                        isHost: payLoad.newHost === profile.name
                    })
                } else {
            ...
            case 'newMessage':
                ...
                setRoom({
                    ...room,
                    message: [...room.message, { body: message, name: send }]
                })
                break
            case 'gameUpdate':
                updateGame(payLoad)
            ...
            case 'gameOver':
                if (payLoad && payLoad.length === 4) {
                    showMessage(
                        `P1: ${payLoad[0]} P2: ${payLoad[1]} P3: ${payLoad[2]} P4: ${payLoad[3]}`,
                        'success',
                        10000
                    )
                }
            ...
            case 'scoreDatas':
                let scores = payLoad.sort((a, b) => {
                    if (b.wins === a.wins) return b.rates - a.rates
    ...
    client.onclose = () => {
        ...
        showMessage(
            'Sorry, you are disconnected, please reload!',
            'error',
            10000
        )
    }
    ...
    const leaveRoom = (roomId, index, players, playersNum) => {
        sendData([
            'leaveRoom',
            { roomId, index, players, playersNum, name: profile.name }
        ])
    }
    ...
    const acceptInvitation = ({ roomId, index, players }) => {
        sendData([
            'acceptInvitation',
            { roomId, index, name: profile.name, players }
        ])
    }
