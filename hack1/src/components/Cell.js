...
export default function Cell({ rowIdx, colIdx, detail, updateFlag, revealCell }) {
	const cellStyle = {
		background: detail.revealed ? (detail.value === '💣' ? '#880000' : mineCheckPattern(detail.x, detail.y)) : checkPattern(detail.x, detail.y),
	...
	return (
		<div onContextMenu={e => updateFlag(e, detail.x, detail.y)} onClick={() => revealCell(detail.x, detail.y)} style={cellStyle} className='cell' id={ID}>
