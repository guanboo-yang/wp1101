...
function Board(props) {
	...
	// TODO 2-(2): fetch all posts from database (/allPosts)
	useEffect(() => {
	...
	return (
		<>
			<div className='board-navbar'>
				...
				<Button className='board-launch-btn' variant='contained' color='primary' id='pid-post-btn' onClick={() => props.navigate('/new')}>
					New Post
				</Button>
			</div>
						...
						{posts.map((post, i) => (
							<div className='article-post' key={i} id={`pid-${i}`}>
								<div className='article-prefix'>
									...
									<span className='each-id' id={`pid-${i}-title`} onClick={() => props.navigate(`/post/${post.postId}`)}>
										{post.title}
									</span>
								</div>
